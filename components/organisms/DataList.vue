...
export default {
  ...
  computed: {
    FileDay() {
      ...
      let day = `${dayArr[0]}/${this.zeroPadding(
        dayArr[1],
        2
      )}/${this.zeroPadding(dayArr[2], 2)}`;
      let time = `${this.zeroPadding(timeArr[0], 2)}:${this.zeroPadding(
        timeArr[1],
        2
      )}`;
      return `日付: ${day}　時刻: ${time}　物語: ${files[3]}. ${
        this.file.story.title
      }　被験者: ${this.file.name}`;
    },
    FileDataTable() {
    ...
    TimePerChar() {
      return this.TimePerWordCount(this.file.story.word_count,this.AllTime());
    },
    TimePerPara() {
      return this.TimePerParagraphs(this.file.story.paragraphs,this.AllTime());
    },
  },
  methods: {
    StayTime(start, end) {
